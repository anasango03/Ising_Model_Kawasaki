...
mag_media_sup=np.zeros(10) #Vector de magnetizaciones de la mitad superior
mag_media_inf=np.zeros(10) #Vector de magnetizaciones de la mitad inferior
...
for t in range(10):    
    ...
    for k in range(l): 
        ...
        #Calculo la magnetización de la mitad superior y de la mitad inferior por separado cada 100pMC  
        if k%(N*N*100)==0:
            matriz_sup = S[:N//2, :]
            matriz_inf = S[N//2:, :]
    ...
    mag_media_sup[t] = mag_sup/((l/100)*(N**2/2))
    mag_media_inf[t] = mag_inf/((l/100)*(N**2/2))
